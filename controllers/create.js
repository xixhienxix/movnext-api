    ...
    if (userExist == false) {
        ...
        if (createdUser == true) {
            ...
            const parametrosQueryResult = await Parametros.insertMany(parametros_default_valuesValuesWithHotelName)
                ...
                Promise.all([amaQueryResult, codigoQueryResult, divisasQueryResult, 
                    estatusQueryResult, estatusBloqueoQueryResult, foliadorQueryResult, 
                    origenQueryResult, parametrosQueryResult])
                .then(
                    () => {
                        res.status(200).send({mensaje:"Usuario agregado con exito"})
                    })
                .catch((err) => {
                    console.error(err);
                });
